...
/**
 * Is this request arriving over the PUBLIC route (anthropic.vegvisr.org/*)
 * rather than through a service binding from a sibling worker?
 *
 * Verified empirically 2026-09-07: a public edge request carries `request.cf`
 * plus `cf-connecting-ip` / `cf-ray`; a service-binding subrequest carries
 * none of them. Either signal present => treat as public and demand a token
 * (fail toward requiring auth, never toward skipping it).
 *
 * A route and a binding are independent doors. Four workers bind this worker
 * (audio-portfolio, realtime, vemotion, api) — the binding never closed the
 * route, which is why anonymous callers could spend the Anthropic key.
 */
function isPublicRequest(request) {
  return Boolean(request.cf) || Boolean(request.headers.get('cf-connecting-ip'))
}

/**
 * Resolve the caller from X-API-Token against the vegvisr_org config table —
 * the ecosystem's standard gate (deployment-status-worker, vemotion-worker,
 * blotato-worker all use this exact lookup).
 */
async function authenticatePublicCaller(request, env) {
  const apiToken = request.headers.get('X-API-Token')
  if (!apiToken) return { valid: false, error: 'Missing X-API-Token header' }
  if (!env.DB) return { valid: false, error: 'Auth database not configured' }
  try {
...
export default {
  async fetch(request, env) {
    ...
    // Routes that cost nothing and expose no caller data stay open.
    const OPEN_PATHS = ['/health', '/api/docs', '/models']

    // Everything else SPENDS the Anthropic key, so a public caller must present
    // a valid X-API-Token. Service-binding callers (sibling workers) pass through
    // untouched — they are already inside the trust boundary.
    if (!OPEN_PATHS.includes(pathname) && isPublicRequest(request)) {
      const auth = await authenticatePublicCaller(request, env)
      if (!auth.valid) {
