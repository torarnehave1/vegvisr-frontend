...
// `extraHeaders` are signed alongside the standard three (host, date, content-sha256).
// Needed by any request whose semantics live in headers rather than the body —
// r2Copy (x-amz-copy-source, x-amz-metadata-directive) and the multipart
// CreateMultipartUpload call (content-type + x-amz-meta-*). Header names are
// lower-cased because SigV4 canonicalization requires it.
async function signR2Request(method, bucket, key, accessKeyId, secretKey, accountId, body = null, extraHeaders = null) {
...
// Rewrite an object's metadata in place. S3 has no "patch metadata" verb — the only
// way is a self-referential COPY with metadata-directive REPLACE, which rewrites the
// object with the new x-amz-meta-* set and drops the old one.
async function r2Copy(bucket, key, metadata, contentType, accessKeyId, secretKey, accountId) {
  const sourcePath = `/${bucket}/${key.split('/').map(encodeURIComponent).join('/')}`
  const extraHeaders = {
  ...
  const { url, headers } = await signR2Request('PUT', bucket, key, accessKeyId, secretKey, accountId, null, extraHeaders)
  return fetch(url, { method: 'PUT', headers })
}

// SigV4 variant that signs a query string as part of the canonical request.
// signR2Request always signs an empty query, which is fine for plain object ops but
// wrong for the multipart calls below — every one of them carries its state in the
// query (`?uploads=`, `?partNumber=&uploadId=`), so the query must be signed or R2
// rejects the signature. Returns { url, headers } rather than performing the fetch.
...
// ── S3 multipart upload over REST ─────────────────────────────────────────────
// The native `env.MEETING_RECORDINGS.createMultipartUpload` binding can only reach
// the shared bucket — a Worker cannot bind a foreign account's R2. Founder uploads
// land in the founder's OWN bucket, so those go over the S3 REST API with the
// founder's own access key, which means hand-rolling the three-call multipart dance.
...
// What may be uploaded into recordings/. Audio is allowed alongside video so a
// podcast/voice recording made in the browser lands in the same library, with the
// same listing, sharing, download and transcription paths as a meeting video —
// nothing downstream inspects the media type, it keys off the R2 object.
function isUploadableRecordingType(contentType) {
  ...
  async fetch(request, env) {
    ...
    if (pathname === '/realtime/recordings/upload/init' && request.method === 'POST') {
      try {
        ...
        if (!isUploadableRecordingType(effectiveType)) {
          return createResponse(JSON.stringify({ error: 'Only video or audio uploads are supported' }), 400)
        }
    ...
    // ── POST /realtime/recordings/upload/direct/* ─────────────────────────────
    // The founder twin of the /upload/* flow above. Identical wire shape (init →
    // part → complete, abort on failure), but writes into the CALLER'S OWN R2
    // bucket over the S3 REST API instead of the shared MEETING_RECORDINGS
    // binding — a Worker cannot bind a foreign account's bucket, so the native
    // multipart API is unavailable here. Open to Admin as well as Superadmin,
    // since this is exactly the path a World Founder uses for their own library.
    if (pathname === '/realtime/recordings/upload/direct/init' && request.method === 'POST') {
      try {
        const auth = await validateWorkerApiToken(request, env)
        if (!auth.valid) return createResponse(JSON.stringify({ error: auth.error }), 401)
        ...
        if (!useOwnR2) {
          return createResponse(JSON.stringify({ error: 'No own R2 bucket configured for this account — ask a Superadmin to set it up, or use the shared upload flow.' }), 400)
        }
        const { filename, contentType, size, folder, title, labels } = await request.json()
        if (!filename || typeof filename !== 'string') return createResponse(JSON.stringify({ error: 'filename is required' }), 400)
        if (!size || typeof size !== 'number') return createResponse(JSON.stringify({ error: 'size is required' }), 400)
        ...
        if (typeof title === 'string' && title.trim()) customMetadata.title = title.trim().slice(0, 200)
        const labelStr = typeof labels === 'string' && labels.trim() ? labels.trim() : ''
        if (labelStr) customMetadata.labels = labelStr.slice(0, 200)
        const created = await r2CreateMultipartUpload(creds.r2Bucket, target.key, effectiveType, customMetadata, creds.r2AccessKeyId, creds.r2Secret, creds.r2AccountId)
        if (!created.ok) return createResponse(JSON.stringify({ error: `R2 CreateMultipartUpload failed (${created.status}): ${created.error}` }), 502)
        return createResponse(JSON.stringify({
          success: true,
        ...
        console.error('Error in /realtime/recordings/upload/direct/init:', e)
        return createResponse(JSON.stringify({ error: e.message }), 500)
      }
    }

    if (pathname === '/realtime/recordings/upload/direct/part' && request.method === 'POST') {
      try {
        const auth = await validateWorkerApiToken(request, env)
        if (!auth.valid) return createResponse(JSON.stringify({ error: auth.error }), 401)
        if (!canManageRecordings(auth.role)) return createResponse(JSON.stringify({ error: 'Admin or Superadmin access required' }), 403)
        const creds = await getUserCloudflareCredentials(auth.email, env)
        const useOwnR2 = !!(creds.r2AccessKeyId && creds.r2Secret && creds.r2Bucket && creds.r2AccountId)
        if (!useOwnR2) return createResponse(JSON.stringify({ error: 'No own R2 bucket configured for this account.' }), 400)
        const partUrl = new URL(request.url)
        const key = String(partUrl.searchParams.get('key') || '')
        const uploadId = String(partUrl.searchParams.get('uploadId') || '')
        const partNumber = Number(partUrl.searchParams.get('partNumber') || '')
        if (!key.startsWith('recordings/')) return createResponse(JSON.stringify({ error: 'Invalid upload key' }), 400)
        if (!uploadId) return createResponse(JSON.stringify({ error: 'uploadId is required' }), 400)
        if (!Number.isInteger(partNumber) || partNumber < 1 || partNumber > 10000) {
          return createResponse(JSON.stringify({ error: 'partNumber must be an integer between 1 and 10000' }), 400)
        ...
        const body = await request.arrayBuffer()
        if (!body.byteLength) return createResponse(JSON.stringify({ error: 'Upload part is empty' }), 400)
        const part = await r2UploadPart(creds.r2Bucket, key, uploadId, partNumber, body, creds.r2AccessKeyId, creds.r2Secret, creds.r2AccountId)
        if (!part.ok) return createResponse(JSON.stringify({ error: `R2 UploadPart failed (${part.status}): ${part.error}` }), 502)
        return createResponse(JSON.stringify({ success: true, part: { partNumber, etag: part.etag } }))
      } catch (e) {
        console.error('Error in /realtime/recordings/upload/direct/part:', e)
        return createResponse(JSON.stringify({ error: e.message }), 500)
      }
    }

    if (pathname === '/realtime/recordings/upload/direct/complete' && request.method === 'POST') {
      try {
        const auth = await validateWorkerApiToken(request, env)
        if (!auth.valid) return createResponse(JSON.stringify({ error: auth.error }), 401)
        if (!canManageRecordings(auth.role)) return createResponse(JSON.stringify({ error: 'Admin or Superadmin access required' }), 403)
        ...
        const normalizedParts = parts
          ...
          .sort((a, b) => a.partNumber - b.partNumber)
        if (!normalizedParts.length) return createResponse(JSON.stringify({ error: 'No valid parts were provided' }), 400)
        const completed = await r2CompleteMultipartUpload(creds.r2Bucket, key, uploadId, normalizedParts, creds.r2AccessKeyId, creds.r2Secret, creds.r2AccountId)
        if (!completed.ok) return createResponse(JSON.stringify({ error: `R2 CompleteMultipartUpload failed (${completed.status}): ${completed.error}` }), 502)
        const playUrl = creds.r2PublicBase ? `${creds.r2PublicBase.replace(/\/+$/, '')}/${key}` : null
        return createResponse(JSON.stringify({
          success: true,
          key,
          name: typeof name === 'string' && name ? name : key.replace(/^recordings\//, ''),
        ...
        console.error('Error in /realtime/recordings/upload/direct/complete:', e)
        return createResponse(JSON.stringify({ error: e.message }), 500)
      }
    }

    if (pathname === '/realtime/recordings/upload/direct/abort' && request.method === 'POST') {
      try {
        const auth = await validateWorkerApiToken(request, env)
        if (!auth.valid) return createResponse(JSON.stringify({ error: auth.error }), 401)
        if (!canManageRecordings(auth.role)) return createResponse(JSON.stringify({ error: 'Admin or Superadmin access required' }), 403)
        ...
        if (!key || typeof key !== 'string' || !key.startsWith('recordings/')) return createResponse(JSON.stringify({ error: 'Valid key is required' }), 400)
        if (!uploadId || typeof uploadId !== 'string') return createResponse(JSON.stringify({ error: 'uploadId is required' }), 400)
        await r2AbortMultipartUpload(creds.r2Bucket, key, uploadId, creds.r2AccessKeyId, creds.r2Secret, creds.r2AccountId)
        return createResponse(JSON.stringify({ success: true }))
      } catch (e) {
        console.error('Error in /realtime/recordings/upload/direct/abort:', e)
        return createResponse(JSON.stringify({ error: e.message }), 500)
      }
    }
    ...
    // ── POST /realtime/recordings/describe ────────────────────────────────────
    // Transcript → { description, category, metaArea } via Claude, so a recording
    // can be filed in the library without the owner writing metadata by hand. The
    // model is asked for raw JSON; a fenced or malformed reply degrades to using
    // the raw text as the description rather than failing the request.
    if (pathname === '/realtime/recordings/describe' && request.method === 'POST') {
      try {
        const auth = await validateWorkerApiToken(request, env)
        if (!auth.valid) return createResponse(JSON.stringify({ error: auth.error }), 401)
        if (!canManageRecordings(auth.role)) return createResponse(JSON.stringify({ error: 'Admin or Superadmin access required' }), 403)
        if (!env.ANTHROPIC) return createResponse(JSON.stringify({ error: 'ANTHROPIC service binding not configured' }), 500)
        const body = await request.json()
        const transcript = String(body.transcript || '').trim()
        const currentTitle = String(body.title || '').trim()
        if (!transcript) return createResponse(JSON.stringify({ error: 'transcript is required' }), 400)
        const MAX_TRANSCRIPT = 12000
        const clipped = transcript.length > MAX_TRANSCRIPT ? transcript.slice(0, MAX_TRANSCRIPT) : transcript
        const system = 'You label meeting/video recordings for a searchable library. Given a transcript, respond with ONLY a raw JSON object — no markdown, no code fences, no prose. Shape exactly: {"description": string (one clear sentence, max 200 chars), "category": string (one or two words, e.g. "Onboarding"), "metaArea": string (2-4 space-separated hashtag tags, e.g. "#Onboarding #HR")}.'
        const userMsg = (currentTitle ? `Working title: ${currentTitle}\n\n` : '') + `Transcript:\n${clipped}`
        const aiResp = await env.ANTHROPIC.fetch('https://anthropic.vegvisr.org/chat', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
            userId: auth.email || 'recordings-describe',
        ...
        if (!aiResp.ok) {
          return createResponse(JSON.stringify({ error: 'AI describe failed', details: aiData?.error || aiData }), 502)
        }
        const textBlock = (aiData?.content || []).find(b => b?.type === 'text')
        let raw = String(textBlock?.text || '').trim()
        raw = raw.replace(/^```(?:json)?\s*/i, '').replace(/\s*```$/, '').trim()
        let parsed = null
        try { parsed = JSON.parse(raw) } catch { /* fall through to raw-text fallback */ }
        const result = parsed && typeof parsed === 'object'
          ? {
              ...
              metaArea: String(parsed.metaArea || '').trim().slice(0, 200),
            }
          : { description: raw.slice(0, 200), category: '', metaArea: '' }
        return createResponse(JSON.stringify({ success: true, ...result, model: aiData?.model || 'claude-haiku-4-5-20251001' }))
      } catch (e) {
        console.error('Error in /realtime/recordings/describe:', e)
        return createResponse(JSON.stringify({ error: e.message }), 500)
      }
    }
    ...
    if (pathname === '/realtime/recordings/download' && request.method === 'GET') {
      try {
        ...
        // exactly as /realtime/recordings lists them and /recordings/shared serves them.
        const eff = await resolveEffectiveEmail(request, auth)
        if (!eff.ok) return createResponse(JSON.stringify({ error: eff.error }), eff.status)
        const creds = await getUserCloudflareCredentials(eff.email, env)
        if (creds.r2AccessKeyId && creds.r2Secret && creds.r2Bucket && creds.r2AccountId) {
    ...
    // ── POST /realtime/recordings/share-token ─────────────────────────────────
    // Mint a 7-day public link for one recording. The recording is verified to
    // exist first (own R2, then shared bucket) so a share link can never point at
    // nothing. The token row in `recording_share_tokens` carries owner_email, so
    // the redeeming GET below can resolve the right bucket without any auth.
    if (pathname === '/realtime/recordings/share-token' && request.method === 'POST') {
      try {
        const auth = await validateWorkerApiToken(request, env)
        if (!auth.valid) return createResponse(JSON.stringify({ error: auth.error }), 401)
        if (!canManageRecordings(auth.role)) return createResponse(JSON.stringify({ error: 'Admin or Superadmin access required' }), 403)
        ...
        if (creds.r2AccessKeyId && creds.r2Secret && creds.r2Bucket && creds.r2AccountId) {
          const headResp = await r2Head(creds.r2Bucket, key, creds.r2AccessKeyId, creds.r2Secret, creds.r2AccountId)
          if (headResp.ok) found = true
        }
        if (!found && env.MEETING_RECORDINGS) {
          const head = await env.MEETING_RECORDINGS.head(key)
          if (head) found = true
        }
        if (!found) return createResponse(JSON.stringify({ error: 'Recording not found' }), 404)
        const tokenId = crypto.randomUUID()
        const now = new Date()
        const validUntil = new Date(now.getTime() + 7 * 24 * 60 * 60 * 1000)
        await env.vegvisr_org.prepare(
          'INSERT INTO recording_share_tokens (id, owner_email, r2_key, file_name, valid_until, created_by, created_at) VALUES (?, ?, ?, ?, ?, ?, ?)'
        ).bind(tokenId, eff.email, key, fileName || key.split('/').pop() || 'recording', validUntil.toISOString(), auth.email, now.toISOString()).run()
        const shareUrl = `https://api.vegvisr.org/realtime/recordings/shared?token=${tokenId}`
        return createResponse(JSON.stringify({ success: true, token: tokenId, url: shareUrl, validUntil: validUntil.toISOString() }))
      } catch (e) {
        console.error('Error in /realtime/recordings/share-token:', e)
        return createResponse(JSON.stringify({ error: e.message }), 500)
      }
    }

    // ── GET /realtime/recordings/shared?token=… ───────────────────────────────
    // Public redemption of the link minted above — deliberately unauthenticated;
    // possession of the token IS the authorization. Streams the object inline so
    // a browser plays it rather than downloading. Revoked and expired tokens are
    // rejected before any bucket read.
    if (pathname === '/realtime/recordings/shared' && request.method === 'GET') {
      try {
        const tokenId = url.searchParams.get('token')
        if (!tokenId) return createResponse(JSON.stringify({ error: 'token query parameter is required' }), 400)
        const row = await env.vegvisr_org.prepare(
          'SELECT owner_email, r2_key, file_name, valid_until, revoked_at FROM recording_share_tokens WHERE id = ?'
        ).bind(tokenId).first()
        if (!row) return createResponse(JSON.stringify({ error: 'Invalid share link' }), 404)
        if (row.revoked_at) return createResponse(JSON.stringify({ error: 'This share link has been revoked' }), 403)
        if (new Date(row.valid_until).getTime() < Date.now()) return createResponse(JSON.stringify({ error: 'This share link has expired' }), 403)
        const key = row.r2_key
        const fileName = row.file_name || key.split('/').pop() || 'recording'
        const creds = await getUserCloudflareCredentials(row.owner_email, env)
        if (creds.r2AccessKeyId && creds.r2Secret && creds.r2Bucket && creds.r2AccountId) {
          const getResp = await r2Get(creds.r2Bucket, key, creds.r2AccessKeyId, creds.r2Secret, creds.r2AccountId)
          if (getResp.ok) {
            return new Response(getResp.body, {
        ...
        if (!env.MEETING_RECORDINGS) return createResponse(JSON.stringify({ error: 'Recording not found' }), 404)
        const obj = await env.MEETING_RECORDINGS.get(key)
        if (!obj) return createResponse(JSON.stringify({ error: 'Recording not found' }), 404)
        return new Response(obj.body, {
          headers: {
        ...
        console.error('Error in /realtime/recordings/shared:', e)
        return createResponse(JSON.stringify({ error: e.message }), 500)
      }
    }
