...
// albums-worker writes a `share:<shareId>` -> album-name pointer whenever sharing is turned on,
// so the common path is one KV read instead of listing every album and reading each one. The scan
// stays as the fallback for albums shared before the pointer existed (five of them, 2026-09-06);
// each of those repairs itself the next time its sharing is toggled through albums-worker.
const findAlbumByShareId = async (env, shareId) => {
  if (!env.PHOTO_ALBUMS) return null
  const pointed = await env.PHOTO_ALBUMS.get(`share:${shareId}`)
  if (pointed) {
    const album = await readPhotoAlbum(env, pointed)
...
const handleListR2Images = async (request, env) => {
  ...
  if ((albumName || shareId) && env.PHOTO_ALBUMS) {
    ...
    // Per-image opt-out (album.hiddenImages, set from the Photos app). It applies to the SHARE-LINK
    // view only — ?album=<name> always returns the whole album, so hiding a photo never hides it
    // from the person who hid it. An owner who opens the share link sees exactly what a visitor
    // sees, which is the preview they need. An album shares everything it holds by default:
    // hiddenImages is empty unless someone deliberately held a photo back.
    const shareLinkView = !!shareId && !!album.isShared
    const hidden = new Set(shareLinkView && Array.isArray(album.hiddenImages) ? album.hiddenImages : [])
    const visibleKeys = (album.images || []).filter((key) => !hidden.has(key))
    const images = await Promise.all(visibleKeys.map(async (key) => enrichImageWithMetadata(env, {
      key,
      url: `${baseUrl}${key}`,
      uploaded: null
    })))
