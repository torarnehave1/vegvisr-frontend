To ensure that the correct JSON is loaded into the textarea when a user selects a version from the version history, you need to verify that the graphJson value is updated properly when a version is selected.
