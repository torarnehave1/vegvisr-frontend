    ...
    <div class="main-content container-fluid">
      <div class="row">
        ...
        <div class="col-md-3 sidebar bg-light border-end" :class="{ collapsed: sidebarCollapsed }">
          <button class="btn btn-secondary w-100 mb-3" @click="toggleSidebar">
            {{ sidebarCollapsed ? 'Show Sidebar' : 'Hide Sidebar' }}
          </button>
          <div v-if="!sidebarCollapsed">
            <!-- Tabs for Form and JSON Editor -->
        ...
        <!-- Main Graph Editor and Info Panel -->
        <div class="col-md-9 main-panel">
          <div class="row">
              ...
              <div class="row">
                <div class="col-12">
                  <label for="jsoneditor" class="form-label"
                    ><strong>Graph Json Editor:</strong></label
                  >
                  <textarea
                    id="JsonEditor"
                    v-model="graphJson"
            ...
            <!-- Graph Editor and Info Section -->
            <div class="col-12 graph-content">
              <div class="row">
                <!-- Graph Editor -->
                <div class="col-md-8 graph-editor p-3">
                  <div
                    id="cy"
                ...
                <!-- Info Section -->
                <div class="col-md-4 info-section p-3 bg-light border-start">
    ...
    if (parsedGraph.nodes && parsedGraph.edges) {
      graphStore.nodes = parsedGraph.nodes.map((node) => ({
        data: {
          id: node.id,
          label: node.label,
          color: parseColor(node.color), // Use color from JSON
          type: node.type || null,
          info: node.info || null,
        },
    ...
    if (response.ok) {
      ...
      graphStore.nodes = graphData.nodes.map((node) => ({
        data: {
          id: node.id,
          label: node.label,
          color: parseColor(node.color), // Use color from database
          type: node.type || null,
          info: node.info || null,
          bibl: Array.isArray(node.bibl) ? node.bibl : [],
        },
