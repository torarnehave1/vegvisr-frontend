Changes Made:
Added a <div> element with v-if="saveMessage" to display the success message.
Styled the message using Bootstrap's alert alert-success classes for visibility.
Verify Changes:
Save the current graph.
Confirm that the success message is displayed at the top of the main content area for 2 seconds.
