      ...
      <div class="row">
        ...
        <div class="col-md-9 main-panel">
          <div class="row">
            ...
            <!-- Graph Editor -->
            <div
              class="col-12 graph-content resizable"
              :style="{ '--graph-height': `${graphHeight}px` }"
            >
...
const graphHeight = ref(window.innerHeight - 300)

const startResizing = (event) => {
  // Capture initial mouse position and graph height
  const startY = event.clientY
  const startHeight = graphHeight.value

  // Calculate and update graph height based on mouse movement
  const updateGraphHeight = (moveEvent) => {
    const deltaY = moveEvent.clientY - startY
    const newHeight = Math.max(200, startHeight + deltaY) // Enforce minimum height
    graphHeight.value = newHeight

    // Update Cytoscape container size
    if (cyInstance.value) {
      try {
        cyInstance.value.resize()
        console.debug(`Graph height updated to: ${newHeight}px`)
      } catch (error) {
        console.error('Error resizing Cytoscape:', error)
      }
    } else {
      console.warn('Cytoscape instance not available during resize')
    }
  }

  // Clean up event listeners on mouse release
  const stopResizing = () => {
    window.removeEventListener('mousemove', updateGraphHeight)
    window.removeEventListener('mouseup', stopResizing)
  }

  // Attach event listeners for resizing
  window.addEventListener('mousemove', updateGraphHeight)
...
.graph-content {
  position: relative;
  height: var(--graph-height, calc(100vh - 300px));
}
