...
fetch(`https://aff-worker.torarnehave.workers.dev/graph-ambassador-status?${graphIdsParam}`)
  ...
  .then((data) => {
    ...
    const graphWithAmbassadors = Object.keys(data.ambassadorStatus || {}).find(
      (graphId) => data.ambassadorStatus[graphId].hasAmbassadors,
    )
    ...
    if (graphWithAmbassadors) {
      ...
      return fetch(
        `https://aff-worker.torarnehave.workers.dev/affiliate-deals-by-graph?graphId=${graphWithAmbassadors}`,
      )
    } else {
  ...
  .then((data) => {
    ...
    if (data.affiliate) {
      ...
      return fetch(
        `https://aff-worker.torarnehave.workers.dev/affiliate-ambassador-graphs?affiliateId=${data.affiliate.affiliateId}`,
      )
      ...
      if (firstAmbassador) {
        console.log('\n=== Step 3: Test Affiliate Ambassador Graphs ===')
        return fetch(
          `https://aff-worker.torarnehave.workers.dev/affiliate-ambassador-graphs?affiliateId=${firstAmbassador.affiliate_id}`,
        )
      }
    }
