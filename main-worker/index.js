...
export default {
  async fetch(request, env) {
    ...
    // === Direct GitHub Issue Creation defaults (easy to spot/change) ===
    // Token should be provided as a secret: `GITHUB_TOKEN_ISSUES`
    const DIRECT_GITHUB_OWNER_DEFAULT = 'torarnehave1'
    const DIRECT_GITHUB_REPO_DEFAULT = 'vegvisr-frontend'
    const directGithubOwner = env.GITHUB_OWNER || DIRECT_GITHUB_OWNER_DEFAULT
    const directGithubRepo = env.GITHUB_REPO || DIRECT_GITHUB_REPO_DEFAULT
    const directGithubToken = env.GITHUB_TOKEN_ISSUES
    ...
    try {
      ...
      if (path === '/verify-email' && method === 'GET') {
        ...
        const emailToken = url.searchParams.get('token') || url.searchParams.get('magic')
        if (!emailToken) {
          ...
          if (
            ...
            try {
              ...
              const redirectTarget =
                url.searchParams.get('redirect') || 'https://www.vegvisr.org/user'
              const redirectUrl = (() => {
                try {
                  const parsed = new URL(redirectTarget)
                  parsed.searchParams.set('email', parsedBody.email)
                  parsed.searchParams.set('token', jwtToken)
                  return parsed.toString()
                } catch {
                  return `https://dashboard.vegvisr.org?email=${encodeURIComponent(
                    parsedBody.email,
                  )}&token=${encodeURIComponent(jwtToken)}`
                }
              })()

              // Redirect to the requested target with email + token
              return addCorsHeaders(
                new Response(null, {
                  status: 302,
                  headers: {
      ...
      if (path === '/github/issues' && method === 'GET') {
        ...
        try {
          if (!directGithubToken) {
            console.error('Missing env.GITHUB_TOKEN_ISSUES (required for GitHub issues listing)')
            return addCorsHeaders(
              new Response(
                JSON.stringify({
                  success: false,
                  error: 'Missing GITHUB_TOKEN_ISSUES in worker env',
                }),
          ...
          // Minimal query passthrough for common use-cases
          // https://docs.github.com/en/rest/issues/issues#list-repository-issues
          const ghUrl = new URL(
            `https://api.github.com/repos/${directGithubOwner}/${directGithubRepo}/issues`,
          )

          const allowedParams = ['state', 'labels', 'since', 'per_page', 'page', 'sort', 'direction']
          for (const key of allowedParams) {
            const value = url.searchParams.get(key)
            if (value) ghUrl.searchParams.set(key, value)
          }

          console.log('Direct GitHub issues target:', `${directGithubOwner}/${directGithubRepo}`)
          console.log('Direct GitHub issues url:', ghUrl.toString())

          const ghRes = await fetch(ghUrl.toString(), {
            method: 'GET',
            headers: {
              Authorization: `Bearer ${directGithubToken}`,
              Accept: 'application/vnd.github+json',
              'X-GitHub-Api-Version': '2022-11-28',
              'User-Agent': 'vegvisr-main-worker',
            },
          })

          const ghText = await ghRes.text()
          console.log('GitHub API response status:', ghRes.status, ghRes.statusText)
          console.log('GitHub API response body:', ghText)

          if (!ghRes.ok) {
            console.error(`Error fetching GitHub issues: ${ghRes.status} ${ghRes.statusText}`)
            return addCorsHeaders(
              new Response(
                JSON.stringify({
                  success: false,
          ...
          let issues
          try {
            issues = JSON.parse(ghText)
          } catch (parseErr) {
            console.error('Failed to parse GitHub issues response as JSON:', parseErr)
            return addCorsHeaders(
              new Response(
                JSON.stringify({
                  success: false,
      ...
      if (path === '/github/create-issue' && method === 'POST') {
        ...
        try {
          ...
          if (!title || !body) {
            console.warn('Missing title or body in request:', { title, body })
            return addCorsHeaders(
          ...
          if (!directGithubToken) {
            console.error('Missing env.GITHUB_TOKEN_ISSUES (required for GitHub issue creation)')
            return addCorsHeaders(
              new Response(
                JSON.stringify({
                  success: false,
          ...
          const apiUrl = `https://api.github.com/repos/${directGithubOwner}/${directGithubRepo}/issues`

          const payload = { title, body, labels: labels || [] }
          console.log('Direct GitHub target:', `${directGithubOwner}/${directGithubRepo}`)
          console.log('Direct GitHub token present:', directGithubToken ? '[REDACTED]' : '[MISSING]')
          console.log('Direct GitHub payload:', JSON.stringify(payload, null, 2))
          ...
          if (!ghRes.ok) {
            console.error(`Error creating GitHub issue: ${ghRes.status} ${ghRes.statusText}`)
            return addCorsHeaders(
              new Response(
                JSON.stringify({
                  success: false,
          ...
          let result
          try {
            result = JSON.parse(ghText)
          } catch (parseErr) {
            console.error('Failed to parse GitHub API response as JSON:', parseErr)
            return addCorsHeaders(
              new Response(
                JSON.stringify({
                  success: false,
          ...
          return addCorsHeaders(
            new Response(JSON.stringify({ success: true, issue: result }), {
              status: 200,
              headers: { 'Content-Type': 'application/json' },
            }),
          )
        } catch (error) {
          console.error('Error creating GitHub issue:', error && error.stack ? error.stack : error)
          return addCorsHeaders(
            new Response(JSON.stringify({ error: 'Failed to create GitHub issue.', details: error && error.message ? error.message : String(error) }), {
              status: 500,
            }),
          )
        }
      ...
      if (path === '/userdata' && method === 'GET') {
        ...
        const hasPhoneColumn = tableInfo.results.some((col) => col.name === 'phone')
        const hasPhoneVerifiedAtColumn = tableInfo.results.some((col) => col.name === 'phone_verified_at')
        ...
        try {
          if (!hasPhoneColumn) {
            console.log('Adding phone column to config table')
            await db.prepare('ALTER TABLE config ADD COLUMN phone TEXT').run()
          }
        } catch (e) {
          console.log('Phone column may already exist:', e.message)
        }

        try {
          if (!hasPhoneVerifiedAtColumn) {
            console.log('Adding phone_verified_at column to config table')
            await db.prepare('ALTER TABLE config ADD COLUMN phone_verified_at INTEGER').run()
          }
        } catch (e) {
          console.log('phone_verified_at column may already exist:', e.message)
        }

        const query = `SELECT user_id, role, data, profileimage, emailVerificationToken, bio, phone, phone_verified_at FROM config WHERE email = ?;`
      ...
      if (path === '/admin/users' && method === 'GET') {
        const rolesParam = url.searchParams.get('roles') || 'Superadmin'
        ...
        if (roles.length === 0) {
          return addCorsHeaders(
            new Response(JSON.stringify({ error: 'roles parameter is required' }), { status: 400 }),
          )
        }

        const placeholders = roles.map(() => '?').join(',')
        const query = `SELECT email, role FROM config WHERE role IN (${placeholders}) ORDER BY email ASC;`

        try {
          const db = env.vegvisr_org
          const result = await db.prepare(query).bind(...roles).all()
          return addCorsHeaders(
            new Response(JSON.stringify({ roles, users: result.results || [] }), { status: 200 }),
          )
        } catch (error) {
      ...
      if (path === '/userdata-from-token' && method === 'GET') {
        const authHeader = request.headers.get('Authorization') || ''
        ...
        if (!token) {
          return addCorsHeaders(
            new Response(JSON.stringify({ error: 'Missing authorization token' }), { status: 401 }),
          )
        }
      ...
      if (path === '/upload' && method === 'POST') {
        try {
          ...
          if (!file || !email) {
            return addCorsHeaders(
              new Response(JSON.stringify({ error: 'Missing file or email' }), {
          ...
          if (!fileExtension) {
            return addCorsHeaders(
              new Response(JSON.stringify({ error: 'Invalid file name or extension' }), {
                status: 400, headers: { 'Content-Type': 'application/json' },
              })
            )
          }
          ...
          `).bind(email, fileUrl, email, fileUrl).run()

          return addCorsHeaders(
            new Response(JSON.stringify({ success: true, fileUrl }), {
      ...
      // List all domains from KV by owner email
      if (path === '/domains/list' && method === 'GET') {
        try {
          const email = url.searchParams.get('email')
          if (!email) {
            return addCorsHeaders(
              new Response(JSON.stringify({ error: 'Missing email parameter' }), {
                status: 400,
              }),
            )
          }
          ...
          // List all keys in the SITE_CONFIGS KV namespace with the site-config prefix
          ...
          console.error('Error fetching domains from KV:', error)
          return addCorsHeaders(
            new Response(
              JSON.stringify({
                success: false,
      ...
      // ============================================
      // PHONE-TO-DOMAIN BRANDING ENDPOINTS
      // ============================================

      // Helper function for Norwegian phone normalization
      function normalizeNorwegianPhone(phone) {
      ...
      // GET /branding/by-phone - Lookup branding for a phone number (used by Flutter app)
      if (path === '/branding/by-phone' && method === 'GET') {
        try {
          let phone = url.searchParams.get('phone')

          if (!phone) {
            return addCorsHeaders(
              new Response(JSON.stringify({ error: 'Phone parameter is required' }), { status: 400 }),
            )
          }
          ...
          if (!phone) {
            return addCorsHeaders(
              new Response(
                JSON.stringify({ error: 'Invalid phone format. Use Norwegian format (+47XXXXXXXX or 8 digits)' }),
                { status: 400 },
              ),
            )
          }

          console.log('Looking up branding for phone:', phone)

          // Look up phone mapping
          const mappingKey = `phone-mapping:${phone}`
          const domain = await env.SITE_CONFIGS.get(mappingKey)
            ...
            return addCorsHeaders(
              new Response(
                JSON.stringify({
                  ...
                  domain: null,
                  branding: null,
                  message: 'No custom branding for this phone number',
                }),
                { status: 200 },
              ),
            )
          ...
          console.log('Found phone mapping:', phone, '->', domain)

          // Fetch full branding config
          const siteConfigKey = `site-config:${domain}`
          const siteConfigData = await env.SITE_CONFIGS.get(siteConfigKey)
            ...
            return addCorsHeaders(
              new Response(
                JSON.stringify({
                  ...
                  domain: domain,
                  branding: null,
                  message: 'Domain mapping exists but branding not configured',
                }),
                { status: 200 },
              ),
            )
          ...
          return addCorsHeaders(
            new Response(
              JSON.stringify({
                ...
                contentFilter: siteConfig.contentFilter || { metaAreas: [] },
                menuConfig: siteConfig.menuConfig || { enabled: false },
              }),
              { status: 200 },
            ),
          )
        } catch (error) {
          console.error('Error in /branding/by-phone:', error)
          return addCorsHeaders(
      ...
      // PUT /phone-mapping - Save phone-to-domain mapping
      if (path === '/phone-mapping' && method === 'PUT') {
        try {
          const body = await request.json()
          const { phone, domain, ownerEmail } = body

          if (!phone || !domain || !ownerEmail) {
            return addCorsHeaders(
              new Response(
                JSON.stringify({ error: 'phone, domain, and ownerEmail are required' }),
                { status: 400 },
              ),
            )
          }

          const normalizedPhone = normalizeNorwegianPhone(phone)
          if (!normalizedPhone) {
            return addCorsHeaders(
              new Response(
                JSON.stringify({ error: 'Invalid Norwegian phone number format' }),
                { status: 400 },
              ),
            )
          }

          // Verify domain belongs to owner
          const siteConfigKey = `site-config:${domain}`
          const siteConfigData = await env.SITE_CONFIGS.get(siteConfigKey)

          if (siteConfigData) {
            const siteConfig = JSON.parse(siteConfigData)
            if (siteConfig.owner !== ownerEmail) {
              return addCorsHeaders(
                new Response(JSON.stringify({ error: 'Domain not owned by this user' }), {
                  status: 403,
                }),
              )
            }
          ...
          // Save mapping
          const mappingKey = `phone-mapping:${normalizedPhone}`
          await env.SITE_CONFIGS.put(mappingKey, domain)

          console.log(`Phone mapping saved: ${normalizedPhone} -> ${domain}`)
          ...
          console.error('Error in PUT /phone-mapping:', error)
          return addCorsHeaders(
            new Response(JSON.stringify({ error: 'Internal server error' }), { status: 500 }),
          )
        }
      ...
      // DELETE /phone-mapping/{phone} - Delete phone mapping
      if (path.startsWith('/phone-mapping/') && method === 'DELETE') {
        try {
          const phone = decodeURIComponent(path.split('/phone-mapping/')[1])
          const normalizedPhone = normalizeNorwegianPhone(phone)
          ...
          const mappingKey = `phone-mapping:${normalizedPhone}`
          await env.SITE_CONFIGS.delete(mappingKey)

          console.log(`Phone mapping deleted: ${normalizedPhone}`)

          return addCorsHeaders(
            new Response(JSON.stringify({ success: true, message: 'Phone mapping deleted' }), {
              status: 200,
            }),
          )
        } catch (error) {
          console.error('Error in DELETE /phone-mapping:', error)
          return addCorsHeaders(
            new Response(JSON.stringify({ error: 'Internal server error' }), { status: 500 }),
          )
        }
      ...
      // GET /phone-mappings?domain={domain} - List phone mappings for a domain
      if (path === '/phone-mappings' && method === 'GET') {
        try {
          ...
          console.log('Listing phone mappings for domain:', domain)

          // List all phone mappings
          const listResult = await env.SITE_CONFIGS.list({ prefix: 'phone-mapping:' })
          const mappings = []

          for (const key of listResult.keys) {
            const mappedDomain = await env.SITE_CONFIGS.get(key.name)
            if (mappedDomain === domain) {
              mappings.push({
                phone: key.name.replace('phone-mapping:', ''),
              })
            }
          }

          console.log(`Found ${mappings.length} phone mappings for ${domain}`)
          ...
          console.error('Error in GET /phone-mappings:', error)
          return addCorsHeaders(
            new Response(JSON.stringify({ error: 'Internal server error' }), { status: 500 }),
          )
        }
      ...
      // GET /brand-users?domain={domain} - List all users for a brand
      if (path === '/brand-users' && method === 'GET') {
        try {
          const domain = url.searchParams.get('domain')

          if (!domain) {
            return addCorsHeaders(
              new Response(JSON.stringify({ error: 'domain parameter is required' }), { status: 400 }),
            )
          }

          console.log('Listing brand users for domain:', domain)

          // List all brand users for this domain
          const listResult = await env.SITE_CONFIGS.list({ prefix: `brand-user:${domain}:` })
          const users = []

          for (const key of listResult.keys) {
            const userData = await env.SITE_CONFIGS.get(key.name)
            if (userData) {
              try {
                users.push(JSON.parse(userData))
              } catch (e) {
                console.warn('Error parsing brand user:', key.name)
              }
            }
          }

          console.log(`Found ${users.length} brand users for ${domain}`)
      ...
      // POST /brand-users - Add a user to a brand
      if (path === '/brand-users' && method === 'POST') {
        try {
          const body = await request.json()
          const { domain, phone, role, ownerEmail } = body

          if (!domain || !phone || !role || !ownerEmail) {
            return addCorsHeaders(
              new Response(
                JSON.stringify({ error: 'domain, phone, role, and ownerEmail are required' }),
                { status: 400 },
              ),
            )
          }
          ...
          if (!normalizedPhone) {
            return addCorsHeaders(
              new Response(JSON.stringify({ error: 'Invalid phone format' }), { status: 400 }),
            )
          }
          ...
          if (!siteConfigData) {
            return addCorsHeaders(
              new Response(JSON.stringify({ error: 'Domain not found' }), { status: 404 }),
            )
          }
          ...
          if (siteConfig.owner !== ownerEmail) {
            return addCorsHeaders(
              new Response(JSON.stringify({ error: 'Not authorized to manage this domain' }), {
                status: 403,
              }),
            )
          }

          // Create brand user entry
          const brandUser = {
            phone: normalizedPhone,
            domain: domain,
            role: role, // 'owner', 'admin', or 'member'
            status: 'active',
            addedBy: ownerEmail,
            addedAt: new Date().toISOString(),
          }

          const brandUserKey = `brand-user:${domain}:${normalizedPhone}`
          await env.SITE_CONFIGS.put(brandUserKey, JSON.stringify(brandUser))

          // Also create phone mapping so they get branding in the app
          const phoneMappingKey = `phone-mapping:${normalizedPhone}`
          await env.SITE_CONFIGS.put(phoneMappingKey, domain)

          console.log(`Added brand user: ${normalizedPhone} to ${domain} as ${role}`)
          ...
          console.error('Error in POST /brand-users:', error)
          return addCorsHeaders(
            new Response(JSON.stringify({ error: 'Internal server error' }), { status: 500 }),
          )
        }
      ...
      // PUT /brand-users/{phone} - Update user role or status
      if (path.startsWith('/brand-users/') && method === 'PUT') {
        try {
          const phone = decodeURIComponent(path.split('/brand-users/')[1])
          const body = await request.json()
          const { domain, role, status, ownerEmail } = body

          if (!domain || !ownerEmail) {
            return addCorsHeaders(
              new Response(
                JSON.stringify({ error: 'domain and ownerEmail are required' }),
                { status: 400 },
              ),
            )
          }
          ...
          // Get existing user
          const brandUserKey = `brand-user:${domain}:${normalizedPhone}`
          const existingData = await env.SITE_CONFIGS.get(brandUserKey)
          ...
          const brandUser = JSON.parse(existingData)

          // Update fields
          if (role) brandUser.role = role
          if (status) brandUser.status = status
          brandUser.updatedAt = new Date().toISOString()
          brandUser.updatedBy = ownerEmail

          await env.SITE_CONFIGS.put(brandUserKey, JSON.stringify(brandUser))

          console.log(`Updated brand user: ${normalizedPhone} in ${domain}`)
          ...
          console.error('Error in PUT /brand-users:', error)
          return addCorsHeaders(
            new Response(JSON.stringify({ error: 'Internal server error' }), { status: 500 }),
          )
        }
      ...
      // DELETE /brand-users/{phone}?domain={domain} - Remove user from brand
      if (path.startsWith('/brand-users/') && method === 'DELETE') {
        try {
          const phone = decodeURIComponent(path.split('/brand-users/')[1])
          const domain = url.searchParams.get('domain')
          ...
          // Delete brand user entry
          const brandUserKey = `brand-user:${domain}:${normalizedPhone}`
          await env.SITE_CONFIGS.delete(brandUserKey)

          // Optionally delete phone mapping too
          const phoneMappingKey = `phone-mapping:${normalizedPhone}`
          await env.SITE_CONFIGS.delete(phoneMappingKey)

          console.log(`Removed brand user: ${normalizedPhone} from ${domain}`)
          ...
          console.error('Error in DELETE /brand-users:', error)
          return addCorsHeaders(
            new Response(JSON.stringify({ error: 'Internal server error' }), { status: 500 }),
          )
        }
      ...
      // POST /brand-invites - Create invitation link
      if (path === '/brand-invites' && method === 'POST') {
        try {
          const body = await request.json()
          const { domain, phone, email, role, ownerEmail, expiresInDays } = body

          if (!domain || !role || !ownerEmail) {
            return addCorsHeaders(
              new Response(
                JSON.stringify({ error: 'domain, role, and ownerEmail are required' }),
                { status: 400 },
              ),
            )
          }
          ...
          if (siteConfig.owner !== ownerEmail) {
            return addCorsHeaders(
              new Response(JSON.stringify({ error: 'Not authorized to create invites for this domain' }), {
                status: 403,
              }),
            )
          }
          ...
          const invite = {
            code: inviteCode,
            domain: domain,
            phone: phone ? normalizeNorwegianPhone(phone) : null,
            email: email || null,
            role: role,
          ...
          const inviteKey = `brand-invite:${inviteCode}`
          await env.SITE_CONFIGS.put(inviteKey, JSON.stringify(invite))

          console.log(`Created brand invite: ${inviteCode} for ${domain}`)
          ...
          return addCorsHeaders(
            new Response(
              JSON.stringify({
                ...
                inviteUrl: `https://vegvisr.org/join/${inviteCode}`,
                deepLink: `hallovegvisr://join/${inviteCode}`,
          ...
          console.error('Error in POST /brand-invites:', error)
          return addCorsHeaders(
            new Response(JSON.stringify({ error: 'Internal server error' }), { status: 500 }),
          )
        }
      ...
      // GET /brand-invites?domain={domain} - List pending invitations
      if (path === '/brand-invites' && method === 'GET') {
        try {
          const domain = url.searchParams.get('domain')
          ...
          console.log('Listing brand invites for domain:', domain)

          // List all invites
          const listResult = await env.SITE_CONFIGS.list({ prefix: 'brand-invite:' })
          const invites = []
          const now = new Date()

          for (const key of listResult.keys) {
            const inviteData = await env.SITE_CONFIGS.get(key.name)
            if (inviteData) {
              try {
                const invite = JSON.parse(inviteData)
                // Filter by domain and only show unused, non-expired invites
                if (invite.domain === domain && !invite.usedAt) {
                ...
                console.warn('Error parsing invite:', key.name)
              }
            }
          }

          console.log(`Found ${invites.length} pending invites for ${domain}`)
          ...
          console.error('Error in GET /brand-invites:', error)
          return addCorsHeaders(
            new Response(JSON.stringify({ error: 'Internal server error' }), { status: 500 }),
          )
        }
      ...
      // GET /brand-invite/{inviteCode} - Get invitation details (public)
      if (path.startsWith('/brand-invite/') && !path.includes('/accept') && method === 'GET') {
        try {
          const inviteCode = path.split('/brand-invite/')[1]

          const inviteKey = `brand-invite:${inviteCode}`
          const inviteData = await env.SITE_CONFIGS.get(inviteKey)

          if (!inviteData) {
            return addCorsHeaders(
              new Response(JSON.stringify({ error: 'Invitation not found' }), { status: 404 }),
            )
          }
          ...
          if (invite.usedAt) {
            return addCorsHeaders(
              new Response(JSON.stringify({ error: 'Invitation has already been used' }), { status: 410 }),
            )
          }
          ...
          if (expiresAt < now) {
            return addCorsHeaders(
              new Response(JSON.stringify({ error: 'Invitation has expired' }), { status: 410 }),
            )
          }

          // Fetch branding for preview
          const siteConfigKey = `site-config:${invite.domain}`
          const siteConfigData = await env.SITE_CONFIGS.get(siteConfigKey)
          const branding = siteConfigData ? JSON.parse(siteConfigData).branding : null
          ...
          console.error('Error in GET /brand-invite:', error)
          return addCorsHeaders(
            new Response(JSON.stringify({ error: 'Internal server error' }), { status: 500 }),
          )
        }
      ...
      // POST /brand-invite/{inviteCode}/accept - Accept invitation
      if (path.match(/^\/brand-invite\/[^/]+\/accept$/) && method === 'POST') {
        try {
          const inviteCode = path.split('/brand-invite/')[1].split('/accept')[0]
          const body = await request.json()
          const { phone } = body

          if (!phone) {
            return addCorsHeaders(
              new Response(JSON.stringify({ error: 'phone is required' }), { status: 400 }),
            )
          }
          ...
          const brandUserKey = `brand-user:${invite.domain}:${normalizedPhone}`
          await env.SITE_CONFIGS.put(brandUserKey, JSON.stringify(brandUser))

          // Create phone mapping
          const phoneMappingKey = `phone-mapping:${normalizedPhone}`
          await env.SITE_CONFIGS.put(phoneMappingKey, invite.domain)

          // Mark invite as used
          invite.usedAt = new Date().toISOString()
          invite.usedBy = normalizedPhone
          await env.SITE_CONFIGS.put(inviteKey, JSON.stringify(invite))

          console.log(`Invite ${inviteCode} accepted by ${normalizedPhone}`)
          ...
          console.error('Error in POST /brand-invite/accept:', error)
          return addCorsHeaders(
            new Response(JSON.stringify({ error: 'Internal server error' }), { status: 500 }),
          )
        }
      ...
      // DELETE /brand-invite/{inviteCode} - Cancel invitation
      if (path.startsWith('/brand-invite/') && method === 'DELETE') {
        try {
          const inviteCode = path.split('/brand-invite/')[1]

          const inviteKey = `brand-invite:${inviteCode}`
          await env.SITE_CONFIGS.delete(inviteKey)

          console.log(`Deleted brand invite: ${inviteCode}`)
          ...
          console.error('Error in DELETE /brand-invite:', error)
          return addCorsHeaders(
            new Response(JSON.stringify({ error: 'Internal server error' }), { status: 500 }),
          )
        }
