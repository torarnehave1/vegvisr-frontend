    ...
    try {
      if (path === "/sve2" && method === "GET") {
        ...
        try {
          ...
          const { changes } = await db.prepare(insertQuery).bind(userId, data.email, data.emailVerificationToken, JSON.stringify({}), data.role).run();
          if (changes === 0) {
      ...
      if (path === "/userdata" && method === "PUT") {
        ...
        const result = await db.prepare(query).bind(email, bio, dataJson, profileimage, bio, dataJson, profileimage).run();
...
// .wrangler/tmp/bundle-5fJ6ry/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-5fJ6ry/middleware-loader.entry.ts
