...
app.get("/verify-email", async (c) => {
  try {
    ...
    try {
      ...
      `;
      const { changes } = await db.prepare(query).bind(user_id, data.email, data.emailVerificationToken, JSON.stringify({})).run();
      if (changes === 0) {
...
app.put("/userdata", async (c) => {
  try {
    ...
    if (!user_id || data === void 0 || profileimage === void 0) {
...
app.post("/upload", async (c) => {
  try {
    ...
    const fileName = generateUniqueFileName(email, fileExtension);
...
// .wrangler/tmp/bundle-yjG8rn/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-yjG8rn/middleware-loader.entry.ts
