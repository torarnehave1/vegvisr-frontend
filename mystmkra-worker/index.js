...
function getCorsHeaders(origin) {
  return {
    ...
    'Access-Control-Allow-Headers': 'Content-Type, x-user-email, x-user-role, Authorization, X-API-Token',
  ...
  async fetch(request, env, ctx) {
    ...
    try {
      ...
      if (url.pathname === '/' && request.method === 'GET') {
        return new Response(
          JSON.stringify({ success: true, status: 'ok', service: 'mystmkra-worker' }),
      ...
      // ── Documents Timeline ─────────────────────────────────────
      if (url.pathname === '/documents-timeline' && request.method === 'GET') {
        return handleDocumentsTimeline(env.MYSTMKRA_DB, corsHeaders)
      }

      // ── Classify all documents ──────────────────────────────────
      if (url.pathname === '/classify-all' && request.method === 'POST') {
        return handleClassifyAll(request, env, corsHeaders)
      }

      // ── Get taxonomy ────────────────────────────────────────────
      if (url.pathname === '/taxonomy' && request.method === 'GET') {
        return handleTaxonomy(env.MYSTMKRA_DB, corsHeaders)
      }
...
async function handleConvertToGraph(request, db, env, corsHeaders) {
  try {
    const apiToken = request.headers.get('X-API-Token') || request.headers.get('x-api-token') || ''
    const body = await request.json()
...
// ── Documents Timeline ────────────────────────────────────────────
async function handleDocumentsTimeline(db, corsHeaders) {
  const { results } = await db.prepare(
    `SELECT id, title, created_at, updated_at, tags, semantic_category
     FROM documents
  ...
  return new Response(
    JSON.stringify({ success: true, total: results.length, documents: results }),
    { status: 200, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
  )
}

// ── Taxonomy ──────────────────────────────────────────────────────
async function handleTaxonomy(db, corsHeaders) {
  const { results } = await db.prepare(
    `SELECT id, semantic_category FROM documents WHERE semantic_category IS NOT NULL`
  ).all()

  const taxonomy = {}
  for (const row of results) {
    try {
      const parsed = JSON.parse(row.semantic_category)
      taxonomy[row.id] = parsed
    } catch (e) {
      taxonomy[row.id] = { primary: row.semantic_category, secondary: [], confidence: 0.7 }
    }
  }

  return new Response(
    JSON.stringify({ success: true, total: Object.keys(taxonomy).length, taxonomy }),
    { status: 200, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
  )
}
...
const CATEGORIES = [
  ...
  'PERSONAL_DEVELOPMENT', 'PHILOSOPHY', 'SYSTEM_INTERNAL',
  'TECHNOLOGY', 'MARKETING', 'OTHER'
]

async function classifyDocument(ai, title, contentSnippet) {
  const prompt = `Classify this document into exactly ONE primary category and up to 2 secondary categories.
...
{"primary":"CATEGORY","secondary":["CAT1"],"confidence":0.85}`
  ...
  try {
    ...
    if (!match) return { primary: 'OTHER', secondary: [], confidence: 0.5 }

    const parsed = JSON.parse(match[0])
    if (!CATEGORIES.includes(parsed.primary)) parsed.primary = 'OTHER'
    parsed.secondary = (parsed.secondary || []).filter(c => CATEGORIES.includes(c))
    return parsed
  } catch (e) {
...
async function handleClassifyAll(request, env, corsHeaders) {
  const url = new URL(request.url)
  const limit = Math.min(parseInt(url.searchParams.get('limit') || '20'), 50)

  // Get unclassified documents
  const { results } = await env.MYSTMKRA_DB.prepare(
    `SELECT id, title, substr(content, 1, 500) as snippet
     FROM documents
  ...
  if (results.length === 0) {
    const { results: countRes } = await env.MYSTMKRA_DB.prepare(
      `SELECT COUNT(*) as total FROM documents WHERE semantic_category IS NOT NULL`
    ).all()
    return new Response(
      JSON.stringify({ success: true, classified: 0, message: 'All documents classified', total: countRes[0].total }),
      { status: 200, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
    )
  }
  ...
  for (const doc of results) {
    const result = await classifyDocument(env.AI, doc.title || 'Untitled', doc.snippet || '')
    await env.MYSTMKRA_DB.prepare(
      `UPDATE documents SET semantic_category = ? WHERE id = ?`
    ).bind(JSON.stringify(result), doc.id).run()
    classified++
  }

  const { results: remaining } = await env.MYSTMKRA_DB.prepare(
    `SELECT COUNT(*) as cnt FROM documents WHERE semantic_category IS NULL`
  ).all()

  return new Response(
    JSON.stringify({ success: true, classified, remaining: remaining[0].cnt }),
    { status: 200, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
  )
}
