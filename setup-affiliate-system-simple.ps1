...
if (Test-Path "aff-worker") {
    ...
    try {
        ...
        Write-Warning "Error deploying aff-worker: $($_.Exception.Message)"
        Write-Info "You may need to run: wrangler deploy manually"
    }
    Set-Location ..
} else {
...
if (Test-Path "main-worker") {
    ...
    try {
        ...
        Write-Warning "Error deploying main-worker: $($_.Exception.Message)"
        Write-Info "You may need to run: wrangler deploy manually"
    }
    Set-Location ..
} else {
...
if (Test-Path "email-worker") {
    ...
    try {
        ...
        Write-Warning "Error deploying email-worker: $($_.Exception.Message)"
        Write-Info "You may need to run: wrangler deploy manually"
    }
    Set-Location ..
} else {
...
try {
    ...
    Write-Warning "Error building frontend: $($_.Exception.Message)"
    Write-Info "You may need to run: npm run build manually"
}
