...
var handleGPT4VisionImage = /* @__PURE__ */ __name(async (request, env) => {
  ...
  try {
    ...
    if (model.startsWith("dall-e")) {
      ...
      const filename2 = `ai-generated/${timestamp2}-${Math.random().toString(36).substring(2, 15)}.png`;
      await env.MY_R2_BUCKET.put(filename2, bytes, {
        httpMetadata: {
      ...
      return createResponse(
        JSON.stringify({
          url: `https://vegvisr.imgix.net/${filename2}`,
          size,
          prompt
        })
      );
  ...
  try {
    ...
    const tokenResponse = await fetch("https://oauth2.googleapis.com/token", {
      ...
      body: JSON.stringify({
        ...
        redirect_uri: request.url.includes("localhost") || request.url.includes("127.0.0.1") ? "http://localhost:5173/auth/google/callback.html" : "https://www.vegvisr.org/auth/google/callback.html"
      })
    });
...
// .wrangler/tmp/bundle-T5VYS8/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-T5VYS8/middleware-loader.entry.ts
