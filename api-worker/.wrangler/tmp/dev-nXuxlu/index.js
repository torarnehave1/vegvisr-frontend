...
var handleGenerateEmailTemplate = /* @__PURE__ */ __name(async (request, env) => {
  try {
...
Question: ${prompt}`;
    ...
    const completion = await client.chat.completions.create({
      ...
      messages: [
        {
          role: "system",
          content: graphContext ? "You are a philosophical AI providing deep insights. Use the provided knowledge graph context to inform your response when relevant, but focus on answering the specific question asked." : "You are a philosophical AI providing deep insights."
        },
...
Question: ${inputText}`;
    ...
    if (returnType === "action") {
      ...
      if (followUpResponse.ok) {
        ...
        followUpQuestion = followUpData.candidates?.[0]?.content?.parts?.[0]?.text || followUpQuestion;
      }
      return createResponse(
        JSON.stringify({
          type: "both",
...
var handleClaudeTest = /* @__PURE__ */ __name(async (request, env) => {
...
var handleAIGenerateMenu = /* @__PURE__ */ __name(async (request, env) => {
  try {
    ...
    const categories = graphMetadata.category ? graphMetadata.category.split("#").map((c) => c.trim()).filter((c) => c) : [];
    const metaAreas = graphMetadata.metaArea ? graphMetadata.metaArea.split("#").map((m) => m.trim()).filter((m) => m) : [];
    ...
    if (categories.length > 0 || metaAreas.length > 0) {
      try {
        ...
        for (const graph of graphsResult.results || []) {
          try {
            const graphData2 = JSON.parse(graph.data || "{}");
            const graphMetadata2 = graphData2.metadata || {};
            const graphCategories = graphMetadata2.category ? graphMetadata2.category.split("#").map((c) => c.trim()).filter((c) => c) : [];
            const graphMetaAreas = graphMetadata2.metaArea ? graphMetadata2.metaArea.split("#").map((m) => m.trim()).filter((m) => m) : [];
            const categoryOverlap = categories.some(
              (cat) => graphCategories.some((gc) => gc.toLowerCase().includes(cat.toLowerCase()))
            );
            const metaAreaOverlap = metaAreas.some(
              (meta) => graphMetaAreas.some((gm) => gm.toLowerCase().includes(meta.toLowerCase()))
            );
    ...
    const contentThemes = graphNodes.filter((node) => node.visible !== false && node.info).map((node) => ({
    ...
    const availableRoutes = [
      ...
      { path: "/graph-canvas", label: "Canvas", icon: "\u{1F3A8}" },
      { path: "/graph-portfolio", label: "Portfolio", icon: "\u{1F4C1}" },
      { path: "/graph-viewer", label: "Viewer", icon: "\u{1F441}\uFE0F" },
      { path: "/search", label: "Search", icon: "\u{1F50D}" },
      { path: "/user", label: "Dashboard", icon: "\u{1F4CA}" },
      { path: "/github-issues", label: "Roadmap", icon: "\u{1F5FA}\uFE0F" },
      { path: "/gnew-viewer", label: "GNew Viewer", icon: "\u{1F9EA}", requiresRole: ["Superadmin"] },
      { path: "/sandbox", label: "Sandbox", icon: "\u{1F527}", requiresRole: ["Superadmin"] }
    ];
...
${similarGraphs.slice(0, 5).map(
      (graph, i) => `${i + 1}. "${graph.title}" (ID: ${graph.id}) - Categories: ${graph.categories.join(", ")} - Meta Areas: ${graph.metaAreas.join(", ")}`
    ).join("\n")}
        ...
        case "template-selector":
          ...
          if (validNodeType) {
            ...
            if (item.requiresRole && Array.isArray(item.requiresRole)) {
              ...
              processed.requiresRole = ["Admin", "Superadmin"];
            }
          } else {
...
}, "handleAIGenerateMenu");
var handleAIGenerateQuotes = /* @__PURE__ */ __name(async (request, env) => {
  try {
    ...
    const contextContent = graphContext.map((node) => {
...
Return a JSON object with the following structure:
{
  "quotes": [
    ...
    const completion = await client.chat.completions.create({
      model: "grok-3-beta",
      temperature: 0.8,
      // Higher temperature for more creative quotes
      max_tokens: 1500,
      messages: [
        {
          role: "system",
          content: "You are an expert at creating inspirational quotes and extracting wisdom from complex content. You excel at distilling key insights into memorable, shareable quotes that resonate with people."
        },
        { role: "user", content: prompt }
      ]
    });
...
var handleGetStyleTemplates = /* @__PURE__ */ __name(async (request) => {
  ...
  const allTemplates = [
    ...
    },
    {
      ...
      description: "Simple formatting with FANCY headers, SECTION blocks, and QUOTE citations - no images",
  ...
  try {
    ...
    if (data.photos && data.photos.length > 0) {
      return data.photos.map((photo) => ({
        ...
        photographer_url: photo.photographer_url || `https://www.pexels.com/@${photo.photographer.toLowerCase().replace(/\s+/g, "-")}`,
        // Photographer profile on Pexels
        pexels_url: photo.url || `https://www.pexels.com/photo/${photo.id}/`,
        // Direct link to photo page on Pexels
        id: photo.id,
        width: photo.width,
        height: photo.height,
...
var searchUnsplashImages = /* @__PURE__ */ __name(async (query, env, count = 1) => {
  ...
  try {
    ...
    if (data.results && data.results.length > 0) {
      return data.results.map((photo) => ({
        ...
        // Use regular size for good quality
        ...
        // For download tracking (required by Unsplash API)
        unsplash_url: `https://unsplash.com/photos/${photo.id}`,
        // Direct link to photo on Unsplash
        width: photo.width,
        height: photo.height,
        // Additional sizes for different use cases
        urls: {
...
var handleUnsplashImageSearch = /* @__PURE__ */ __name(async (request, env) => {
  if (!env.UNSPLASH_ACCESS_KEY) {
...
var handleUnsplashDownloadTracking = /* @__PURE__ */ __name(async (request, env) => {
  if (!env.UNSPLASH_ACCESS_KEY) {
    return createErrorResponse("Unsplash Access Key not configured", 500);
  }
  let body;
  ...
  try {
    console.log("\u{1F4CA} Tracking Unsplash download:", download_location);
...
}, "handleUnsplashDownloadTracking");
...
var handleDeploySandbox = /* @__PURE__ */ __name(async (request, env) => {
  try {
    ...
    console.log("\u{1F50D} Original code analysis:");
    ...
    console.log("\u{1F9F9} After cleaning:");
    ...
    if (!cleanCode.includes('addEventListener("fetch"') && !cleanCode.includes("addEventListener('fetch'")) {
      return createErrorResponse(
    ...
    console.log("\u{1F4E4} Final code to deploy:");
    console.log("- Length:", cleanCode.length);
    console.log("- Preview:", JSON.stringify(cleanCode.substring(0, 200) + "..."));
    console.log("\u{1F50D} Character analysis around position 87:");
    ...
    const sandboxMetadata = {
      ...
      codePreview: cleanCode.substring(0, 500),
...
var handleGetSandboxCode = /* @__PURE__ */ __name(async (request, env) => {
  try {
    const url = new URL(request.url);
    const userToken = url.searchParams.get("userToken");
    if (!userToken) {
...
}, "handleGetSandboxCode");
var handleGenerateWorker = /* @__PURE__ */ __name(async (request, env) => {
  try {
    const body = await request.json();
...
Please generate only the JavaScript code for the worker, without any markdown formatting, explanations, or additional text.`;
      ...
      case "grok":
        ...
        const grokCompletion = await grokClient.chat.completions.create({
          ...
          messages: [
            {
              role: "system",
              content: "You are an expert Cloudflare Worker developer. Generate clean, production-ready JavaScript code without any markdown formatting or explanations."
            },
    ...
    let cleanCode = result.replace(/```javascript\n?/g, "").replace(/```js\n?/g, "").replace(/```\n?/g, "").trim();
...
var handleGenerateYouTubeScript = /* @__PURE__ */ __name(async (request, env) => {
  try {
    const body = await request.json();
    const {
      markdown,
    ...
    const finalPrompt = isNorwegian ? `Du er en profesjonell YouTube-skaper og manusforfatter. Generer et omfattende, engasjerende YouTube-manus basert p\xE5 f\xF8lgende dokumentasjon:

DOKUMENTASJON:
${markdown}

VIDEOSTIL: ${scriptStyle || "tutorial"}
M\xC5LVARIGHET: ${targetDuration || "5-10 minutter"}
YOUTUBE URL: ${youtubeUrl || "Ikke oppgitt"}

KRAV:
1. **Huk (F\xF8rste 15 sekunder)** - Fang oppmerksomhet umiddelbart
2. **Verdil\xF8fte** - Fortell seerne hva de vil l\xE6re
3. **Strukturerte seksjoner** med klare overganger
4. **Engasjementselementer** - Abonner-p\xE5minnelser, kommentarer, liker
...
${includeEngagement ? "INKLUDER ENGASJEMENT: Legg til abonner-oppfordringer, like-p\xE5minnelser, kommentarsp\xF8rsm\xE5l" : ""}

FORMAT:
- Profesjonell, samtaleaktig tone
- Klare seksjonsoverskrifter
- Handlingsrettet innhold
- Seer-fokusert spr\xE5k ("du vil l\xE6re", "la meg vise deg")
- Naturlige overganger mellom seksjoner

Generer et komplett, klart-til-bruk YouTube-manus som ville fungere godt for pedagogisk innhold om den dokumenterte funksjonen eller systemet. Skriv HELE manuset p\xE5 norsk.` : `You are a professional YouTube creator and scriptwriter. Generate a comprehensive, engaging YouTube script based on the following documentation:
      ...
      case "grok":
        ...
        const grokCompletion = await grokClient.chat.completions.create({
          ...
          messages: [
            {
              role: "system",
              content: isNorwegian ? "Du er en profesjonell YouTube-skaper og manusforfatter. Lag engasjerende, pedagogiske manus som holder seere interesserte og l\xE6rer dem noe." : "You are a professional YouTube creator and scriptwriter. Create engaging, educational scripts that keep viewers watching and learning."
            },
            { role: "user", content: finalPrompt }
          ]
        });
...
}, "handleGenerateYouTubeScript");
var handleUpdateSandman = /* @__PURE__ */ __name(async (request, env) => {
  try {
    const accountId = env.CF_ACCOUNT_ID;
...
var validateSuperadminRole = /* @__PURE__ */ __name(async (request, email, env) => {
  try {
    const userRole = request.headers.get("x-user-role");
...
var handleAdminDomains = /* @__PURE__ */ __name(async (request, env) => {
  try {
    const url = new URL(request.url);
...
var handleTransferDomain = /* @__PURE__ */ __name(async (request, env) => {
  try {
    ...
    parsedConfig.lastModified = (/* @__PURE__ */ new Date()).toISOString();
    ...
    if (oldOwner) {
      const oldOwnerData = await env.vegvisr_org.prepare("SELECT data FROM config WHERE email = ?").bind(oldOwner).first();
      if (oldOwnerData && oldOwnerData.data) {
        const userData2 = JSON.parse(oldOwnerData.data);
        if (userData2.domainConfigs) {
          userData2.domainConfigs = userData2.domainConfigs.filter((d) => d.domain !== domain);
          await env.vegvisr_org.prepare("UPDATE config SET data = ? WHERE email = ?").bind(JSON.stringify(userData2), oldOwner).run();
        }
      }
    }
    const newOwnerData = await env.vegvisr_org.prepare("SELECT data FROM config WHERE email = ?").bind(newOwner).first();
    ...
    if (!userData.domainConfigs.find((d) => d.domain === domain)) {
      userData.domainConfigs.push({
        domain,
        owner: newOwner,
        createdAt: parsedConfig.createdAt || (/* @__PURE__ */ new Date()).toISOString(),
    ...
    `
    ).bind(newOwner, JSON.stringify(userData), JSON.stringify(userData)).run();
    return createResponse(
      JSON.stringify({
        success: true,
...
var handleShareTemplate = /* @__PURE__ */ __name(async (request, env) => {
  try {
    ...
    const targetOwnerData = await env.vegvisr_org.prepare("SELECT data FROM config WHERE email = ?").bind(targetOwner).first();
    let userData = { domainConfigs: [] };
    ...
    await env.vegvisr_org.prepare(
      ...
      ON CONFLICT(email) DO UPDATE SET data = ?
    `
    ).bind(targetOwner, JSON.stringify(userData), JSON.stringify(userData)).run();
    return createResponse(
      JSON.stringify({
        success: true,
...
var handleRemoveDomain = /* @__PURE__ */ __name(async (request, env) => {
  try {
    const url = new URL(request.url);
    const email = url.searchParams.get("email");
    ...
    if (owner) {
      const ownerData = await env.vegvisr_org.prepare("SELECT data FROM config WHERE email = ?").bind(owner).first();
      if (ownerData && ownerData.data) {
        ...
        if (userData.domainConfigs) {
          ...
          await env.vegvisr_org.prepare("UPDATE config SET data = ? WHERE email = ?").bind(JSON.stringify(userData), owner).run();
        }
      }
    }
    return createResponse(
...
// .wrangler/tmp/bundle-CSi56c/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-CSi56c/middleware-loader.entry.ts
