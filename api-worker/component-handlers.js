import { VEGVISR_AUTH_COMPONENT } from './vegvisr-auth-component.js'
...
export async function handleServeComponent(request, env, pathname) {
  try {
    ...
    // Serve vegvisr-auth from its own module BEFORE building the legacy inline `components`
    // object below — that object currently throws while evaluating an unescaped interpolation
    // in another component (knowledge-graph-viewer: `${this.apiEndpoint}`), which would 500 the
    // whole endpoint. Keeping this early return isolates the standard auth component from that
    // pre-existing bit-rot. (TODO: fix the legacy escaping separately.)
    if (componentKey === 'vegvisr-auth') {
      return new Response(VEGVISR_AUTH_COMPONENT, {
        status: 200,
        headers: {
          'Access-Control-Allow-Origin': '*',
          'Content-Type': 'application/javascript',
    ...
    // Registry-graph fallback (SSOT): serve a component authored in the Component
    // Registry graph — the node's metadata.impl IS the executable JS, so editing
    // the graph node changes the served bytes everywhere. Opt-in per node via
    // metadata.delivery containing 'graph-js', so HTML-fragment impls (e.g.
    // theme-toggle) are never served as broken JS. Runs BEFORE the legacy inline
    // `components` object below, which throws on pre-existing unescaped bit-rot.
    if (env.KNOWLEDGE) {
      try {
        const REGISTRY_GRAPH_ID = '4072b898-f111-42a9-b5ca-0d901bb17d26'
        const gr = await env.KNOWLEDGE.fetch(
          `https://knowledge.vegvisr.org/getknowgraph?id=${REGISTRY_GRAPH_ID}`,
        )
        if (gr.ok) {
          const graph = await gr.json()
          const node = (graph?.nodes || []).find(
            (n) =>
              (n.type === 'component' || n.type === 'layout') &&
              String(n.label || '').trim() === componentKey,
          )
          const impl = node?.metadata?.impl
          const delivery = String(node?.metadata?.delivery || '')
          if (impl && delivery.includes('graph-js')) {
        ...
        console.log('Registry-graph component fallback failed:', registryError.message)
      }
    }
