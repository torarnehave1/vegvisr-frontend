...
import { handleMintPublishToken, handleMintContactRouteToken } from './html-publish-token.js'
...
const corsHeaders = {
  ...
  'Access-Control-Allow-Headers':
    'Content-Type, Authorization, x-user-id, x-user-role, X-API-Token, x-user-email, X-Email',
}
...
// --- Unsplash Image Search (restored 2026-06-18: route was dropped while the
// UNSPLASH_ACCESS_KEY var stayed, so /unsplash-search 404'd while Pexels worked) ---
...
// ── Worker AI (/worker-ai/*) — Gemma via the worker's own Workers AI binding ──
// All chat requests execute on Gemma regardless of the provider/model the
// client asks for; the response reports the model actually used. Same model
...
const validateWorkerApiToken = async (request, env) => {
  ...
  try {
    ...
    const tokenRecord = await env.vegvisr_org
      ...
      .first()
    if (!tokenRecord) {
      return { valid: false, error: 'Invalid X-API-Token' }
    }
    ...
    let scopes = []
    try {
      scopes = tokenRecord.scopes ? JSON.parse(tokenRecord.scopes) : []
    } catch {
      scopes = []
    }
    const hasScope =
      scopes.includes('*') ||
      scopes.includes('ai:*') ||
      scopes.includes('ai:chat') ||
      scopes.includes('workers:ai:chat')
    if (!hasScope) {
      return { valid: false, error: 'Token missing required scope: ai:chat' }
    }
    return { valid: true, userId: tokenRecord.user_id, role: 'Token', source: 'api_tokens' }
  } catch (error) {
...
const handleWorkerAIChat = async (request, env) => {
  const auth = await validateWorkerApiToken(request, env)
  if (!auth.valid) {
    return createResponse(JSON.stringify({ success: false, error: auth.error }), 401)
  }
  if (!env.AI) {
    return createResponse(JSON.stringify({ success: false, error: 'AI binding not configured' }), 503)
  }
  let body
  try {
    body = await request.json()
  } catch {
    return createResponse(JSON.stringify({ success: false, error: 'Invalid JSON body' }), 400)
  }
  const inputMessages = Array.isArray(body.messages) ? body.messages : []
  if (inputMessages.length === 0) {
    return createResponse(JSON.stringify({ success: false, error: 'messages array is required' }), 400)
  }
  const messages = []
  ...
  const aiParams = {
    ...
    max_tokens: Number.isFinite(Number(body.max_tokens)) ? Number(body.max_tokens) : 4096,
  }
  if (body.temperature !== undefined && Number.isFinite(Number(body.temperature))) {
    aiParams.temperature = Number(body.temperature)
  }
  let resp
  try {
    resp = await env.AI.run(WORKER_AI_GEMMA_MODEL, aiParams)
  } catch (error) {
    console.error('Workers AI error:', error)
    return createResponse(
      JSON.stringify({ success: false, error: 'AI request failed: ' + (error?.message || 'unknown') }),
      502,
    )
  }
  // Read content ONLY — not .reasoning. Falls back to resp.response.
  const choice = resp?.choices?.[0]?.message || {}
  const content = (choice.content || resp?.response || '').toString().trim()
  if (!content) {
    return createResponse(JSON.stringify({ success: false, error: 'AI returned empty response' }), 502)
  }
  return createResponse(
    JSON.stringify({
      success: true,
...
const handleWorkerAIModels = async () => {
  return createResponse(
    JSON.stringify({
      success: true,
      models: WORKER_AI_MODELS_MAP,
      endpoint: '/worker-ai/chat',
      method: 'POST',
      requiredHeader: 'X-API-Token',
    }),
  )
}

const handleWorkerAIProviders = async () => {
  return createResponse(
    JSON.stringify({
      success: true,
      providers: Object.keys(WORKER_AI_MODELS_MAP),
      endpoint: '/worker-ai/chat',
      method: 'POST',
      requiredHeader: 'X-API-Token',
      note: 'All requests are served by Gemma on Cloudflare Workers AI regardless of requested provider/model.',
      bodyExample: {
...
export default {
  async fetch(request, env) {
    ...
    if (pathname === '/get-auth-token' && request.method === 'GET') {
      try {
        const email = request.headers.get('X-Email')
        const existingToken = request.headers.get('X-API-Token')
        if (!email && !existingToken) {
          return new Response(
            JSON.stringify({ error: 'Authentication required. Provide X-Email or X-API-Token header.' }),
            { status: 401, headers: { 'Content-Type': 'application/json', ...corsHeaders } }
          )
        }
        let userRecord
        if (email) {
          userRecord = await env.vegvisr_org
            .prepare('SELECT emailVerificationToken, email, user_id FROM config WHERE email = ? LIMIT 1')
            .bind(email)
            .first()
        } else {
          userRecord = await env.vegvisr_org
            .prepare('SELECT emailVerificationToken, email, user_id FROM config WHERE emailVerificationToken = ? LIMIT 1')
            .bind(existingToken)
            .first()
        }
        if (!userRecord) {
    ...
    // Proxy /chat-history/* and /agent-sessions to chat-history-worker service binding
    if (pathname.startsWith('/chat-history') || pathname === '/agent-sessions') {
      let chatPath = pathname
      if (pathname.startsWith('/chat-history')) {
        chatPath = pathname.replace('/chat-history', '') || '/'
      }
      const searchParams = url.search ? '?' + url.searchParams.toString() : ''
      return await env.CHAT_HISTORY_WORKER.fetch(
        new Request('http://chat-history-worker' + chatPath + searchParams, {
          method: request.method,
          headers: request.headers,
    ...
    if (pathname === '/worker-ai/chat' && request.method === 'POST') {
      return await handleWorkerAIChat(request, env)
    }
    if (pathname === '/worker-ai/models' && request.method === 'GET') {
      return await handleWorkerAIModels()
    }
    if (pathname === '/worker-ai/providers' && request.method === 'GET') {
      return await handleWorkerAIProviders()
    }
    ...
    if (pathname === '/api/html/publish-token' && request.method === 'POST') {
      return await handleMintPublishToken(request, env)
    }
    if (pathname === '/api/contact/route-token' && request.method === 'POST') {
      return await handleMintContactRouteToken(request, env)
    }
    // Serve reusable Web Components (custom elements) from the components registry.
    // R2 (WEB_COMPONENTS) first, then inline fallback in component-handlers.js.
    // Route was present in earlier versions, dropped in a refactor — revived here.
    if (pathname.startsWith('/components/') && request.method === 'GET') {
    ...
    if (pathname === '/unsplash-search' && (request.method === 'POST' || request.method === 'GET')) {
    ...
    // ── GET /list-meeting-recordings ──────────────────────────────────────────
    // Lists objects from the shared meeting-recordings R2 bucket so the
    // GNewViewer's "Browse R2 Videos" picker can show what's available.
    // Returns each object plus its customMetadata (meetingTitle, duration,
    // syncedAt, etc.) and a playable URL via realtimevideos.vegvisr.org.
    if (pathname === '/list-meeting-recordings' && request.method === 'GET') {
      try {
        const url = new URL(request.url)
        const prefix = url.searchParams.get('prefix') || 'recordings/'
        const limit = Math.min(Math.max(parseInt(url.searchParams.get('limit') || '200', 10) || 200, 1), 1000)
        if (!env.MEETING_RECORDINGS) {
          return new Response(JSON.stringify({ error: 'MEETING_RECORDINGS binding not configured' }), {
            status: 500,
            headers: { 'Content-Type': 'application/json', ...corsHeaders },
          })
        }
        const listing = await env.MEETING_RECORDINGS.list({ prefix, limit, include: ['customMetadata', 'httpMetadata'] })
        const recordings = (listing.objects || []).map(obj => {
          ...
          // characters still resolve on realtimevideos.vegvisr.org. Don't
          // encode the slashes themselves.
          const encodedPath = obj.key.split('/').map(encodeURIComponent).join('/')
          return {
            key: obj.key,
            // `path` is what the GNewViewer node stores; the viewer resolves it
            // against https://realtimevideos.vegvisr.org/ at render time.
            path: obj.key,
            url: `https://realtimevideos.vegvisr.org/${encodedPath}`,
            thumbnailUrl: meta.thumbnailUrl || null,
            fileName: obj.key.replace(prefix, ''),
            title: meta.title || meta.meetingTitle || '',
            meetingTitle: meta.meetingTitle || '',
            meetingId: meta.meetingId || '',
            rtkRecordingId: meta.rtkRecordingId || '',
            duration,
            size: obj.size,
        ...
        console.error('Error in /list-meeting-recordings:', e)
