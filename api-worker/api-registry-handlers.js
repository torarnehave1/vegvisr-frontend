 ...
 * GET /api/apis/search?q=graph&status=active
 */
export async function searchAPIs(request, env) {
  try {
    const url = new URL(request.url)
    ...
    const result = await env.vegvisr_org.prepare(
      `SELECT name, slug, description, icon, color, function_name, function_signature, endpoint_url, example_code
       FROM apiForApps
       WHERE status = ?
         AND (
           name LIKE ? OR slug LIKE ? OR description LIKE ? OR function_signature LIKE ?
           OR endpoint_url LIKE ? OR function_name LIKE ?
         )
       ORDER BY name
       LIMIT 25`
    ).bind(
    ...
    console.error('Search APIs error:', error)
    return createErrorResponse(`Failed to search APIs: ${error.message}`, 500)
  }
}
