...
function generateStaticHTML(options) {
  ...
  // Fall back to the first image found in the graph itself (image node,
  // IMAGEQUOTE background, markdown or inline <img>) when no OG image was
  // picked in the editor - otherwise a graph whose only picture lives inside
  // an IMAGEQUOTE shares with the generic default image.
  const resolvedOgImage = ogImage && ogImage.startsWith('http')
    ? ogImage
    : extractGraphImage(graphData)

  let imageUrl = defaultImage
  if (resolvedOgImage && resolvedOgImage.startsWith('http')) {
    if (resolvedOgImage.includes('vegvisr.imgix.net')) {
      // For imgix URLs, extract the base URL and apply proper OG dimensions
      const baseUrl = resolvedOgImage.split('?')[0] // Remove existing parameters
  ...
  nodesToRender.forEach(node => {
    if (includeHiddenNodes || node.visible !== false) {
      ...
      // Truncate the SOURCE (not the rendered HTML) so a cut never lands
      // inside an element's markup or inside a generated tag.
      const rawInfo = typeof maxNodeInfoLength === 'number'
        ? String(node.info || '').substring(0, maxNodeInfoLength)
        : node.info || ''
      const nodeInfo = renderNodeInfo(rawInfo)
...
/**
 * Render a node's info field for the static page.
 *
 * Everything is escaped by default; recognised fulltext elements are turned
 * into real markup so the SEO page shows what the editor shows instead of the
 * raw [ELEMENT ...] source. Currently handles IMAGEQUOTE - other elements
 * still fall through as plain text.
 */
function renderNodeInfo(info) {
  const source = String(info || '')
  if (!source.trim()) return ''

  const imageQuoteRegex = /\[IMAGEQUOTE\s*([^\]]*)\]([\s\S]*?)\[END\s+IMAGEQUOTE\]/gi
  let html = ''
  let lastIndex = 0
  let match

  while ((match = imageQuoteRegex.exec(source)) !== null) {
    html += renderPlainText(source.slice(lastIndex, match.index))
    html += renderImageQuote(match[1], match[2])
...
/**
 * Escape a run of text and wrap its blank-line-separated blocks in <p>.
 */
...
/**
 * Parse IMAGEQUOTE params - parity with parseImageQuoteParams in the editor
 * and GNewDefaultNode: key:'value' | key:"value" | key:value pairs.
 */
function parseImageQuoteParams(params) {
  const parsed = {}
  const regex = /(\w+):\s*(?:'([^']*)'|"([^"]*)"|([^\s]+))/g
  let m
  while ((m = regex.exec(String(params || ''))) !== null) {
    parsed[m[1]] = m[2] || m[3] || m[4] || ''
  }
  return parsed
}

/**
 * Render one IMAGEQUOTE as a <figure> with a REAL <img> (crawlable by image
 * search, unlike a CSS background) plus the quote text as live text.
 */
function renderImageQuote(params, content) {
  const p = parseImageQuoteParams(params)
  const text = String(content || '').trim()
  const image = p.backgroundImage && /^https?:\/\//i.test(p.backgroundImage) ? p.backgroundImage : ''

  const figureStyle = [
    `aspect-ratio: ${p.aspectRatio || '16/9'}`,
    `width: ${p.width || '100%'}`,
    p.height && p.height !== 'auto' ? `height: ${p.height}` : null,
  ].filter(Boolean).join('; ')

  const quoteStyle = [
    `text-align: ${p.textAlign || 'center'}`,
    `padding: ${p.padding || '2rem'}`,
  ...
  return `<figure class="image-quote" style="${figureStyle}">${img}<blockquote style="${quoteStyle}">${escapeHtml(text)}</blockquote>${cite}</figure>`
}

/**
 * Find the first usable image in a graph, for og:image fallback. Checks image
 * nodes, IMAGEQUOTE backgrounds, markdown images and inline <img> tags, in
 * node order.
 */
function extractGraphImage(graphData) {
  const nodes = graphData?.nodes || []

  const imageNode = nodes.find(n => n?.type === 'image' && typeof n.path === 'string' && n.path.startsWith('http'))
  if (imageNode) return imageNode.path

  for (const node of nodes) {
    const info = String(node?.info || '')
    if (!info) continue

    const quoteMatch = info.match(/\[IMAGEQUOTE\s*([^\]]*)\]/i)
    if (quoteMatch) {
      const bg = parseImageQuoteParams(quoteMatch[1]).backgroundImage
      if (bg && /^https?:\/\//i.test(bg)) return bg
    }

    const mdMatch = info.match(/!\[[^\]]*\]\((https?:\/\/[^)\s]+)/i)
    if (mdMatch) return mdMatch[1]

    const htmlMatch = info.match(/<img[^>]+src=["'](https?:\/\/[^"']+)["']/i)
    if (htmlMatch) return htmlMatch[1]
  }

  for (const node of nodes) {
    if (typeof node?.path === 'string' && /^https?:\/\/.*\.(jpg|jpeg|png|gif|webp|svg)(\?|$)/i.test(node.path)) {
      return node.path
    }
  }
