    ...
    try {
          ...
          paths: {
            ...
            '/getknowgraph': {
              get: {
                ...
                description: 'Retrieves a knowledge graph with all its nodes and edges.',
            ...
            '/getknowgraphs': {
              get: {
                ...
                description: 'Returns a list of all knowledge graphs (id and title only). May be filtered by meta area based on site configuration.',
            ...
            '/deleteknowgraph': {
              post: {
                ...
                description: 'Permanently deletes a graph and all its version history. Requires graph:delete scope.',
            ...
            '/getTemplates': {
              get: {
                ...
                description: 'Returns available graph templates.',
            ...
            schemas: {
              SaveGraphRequest: {
                ...
                properties: {
                  id: {
                    type: 'string',
                    description: 'Unique graph ID (e.g., graph_1234567890)'
                  },
      ...
      if (pathname === '/saveGraphWithHistory' && request.method === 'POST') {
        ...
        try {
          ...
          if (existingGraph) {
            // Update existing graph (don't overwrite user_id/source_app if not provided)
      ...
      if (pathname === '/patchNode' && request.method === 'POST') {
        ...
        try {
          const { graphId, nodeId, fields } = await request.json()

          if (!graphId || !nodeId || !fields || typeof fields !== 'object') {
            return new Response(
              JSON.stringify({ error: 'graphId, nodeId, and fields (object) are required.' }),
              { status: 400, headers: corsHeaders }
            )
          }
          ...
          // 3. Patch only the specified fields (don't allow changing id)
      ...
      if (pathname === '/addTemplate' && request.method === 'POST') {
        ...
        try {
          const requestBody = await request.json()
          const { name, node, ai_instructions, category, userId, tool } = requestBody
