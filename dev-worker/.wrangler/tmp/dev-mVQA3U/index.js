...
// .wrangler/tmp/bundle-IEXY87/middleware-loader.entry.ts
init_modules_watch_stub();

// .wrangler/tmp/bundle-IEXY87/middleware-insertion-facade.js
init_modules_watch_stub();
...
function generateGraphHtml(graphData, graphId) {
  ...
  const nodesHtml = nodes.map((node) => {
    ...
    let processedContent = nodeInfo.replace(/\[SECTION[^\]]*\](.*?)\[END SECTION\]/gs, "$1").replace(/\[QUOTE[^\]]*\](.*?)\[END QUOTE\]/gs, "<blockquote>$1</blockquote>").replace(/\[FANCY[^\]]*\](.*?)\[END FANCY\]/gs, "<strong>$1</strong>").replace(
      /!\[([^\]]*)\|[^\]]*\]\(([^)]+)\)/g,
      '<img src="$2" alt="$1" style="max-width: 100%; height: auto;" />'
    ).replace(/\*\*(.*?)\*\*/g, "<strong>$1</strong>").replace(/_(.*?)_/g, "<em>$1</em>").replace(/\n\n/g, "</p><p>").replace(/\n/g, "<br>");
    if (processedContent.trim()) {
      processedContent = "<p>" + processedContent + "</p>";
    }
    const biblHtml = nodeBibl.length > 0 ? `
...
__name(generateGraphHtml, "generateGraphHtml");
var index_default = {
  async fetch(request, env) {
    const corsHeaders = {
      "Access-Control-Allow-Origin": request.headers.get("Origin") || "*",
      "Access-Control-Allow-Methods": "GET, POST, OPTIONS",
      "Access-Control-Allow-Headers": "Content-Type, x-user-role, X-API-Token, Accept, Origin, Cache-Control",
    ...
    try {
      ...
      if (pathname === "/saveknowgraph" && request.method === "POST") {
        ...
        try {
          await env.vegvisr_org.prepare("UPDATE knowledge_graphs SET ai_instructions = ?, updated_at = ? WHERE id = ?").bind(instructions, (/* @__PURE__ */ new Date()).toISOString(), graphId).run();
      ...
      if (pathname === "/updateknowgraph" && request.method === "POST") {
        try {
          ...
          await env.vegvisr_org.prepare(query).bind(JSON.stringify(graphData), (/* @__PURE__ */ new Date()).toISOString(), id).run();
        ...
        try {
          ...
          if (existingGraph) {
            const updateGraphQuery = `
              ...
              WHERE id = ?
            `;
            await env.vegvisr_org.prepare(updateGraphQuery).bind(
            ...
            const insertGraphQuery = `
              ...
              VALUES (?, ?, ?, ?, ?, ?, ?)
            `;
            await env.vegvisr_org.prepare(insertGraphQuery).bind(
              id,
              enrichedGraphData.metadata.title,
              enrichedGraphData.metadata.description,
              enrichedGraphData.metadata.createdBy,
      ...
      if (pathname === "/duplicateknowgraph" && request.method === "POST") {
        try {
          ...
          const insertMainQuery = `
            ...
            VALUES (?, ?, ?, ?, ?, ?, ?)
          `;
          await env.vegvisr_org.prepare(insertMainQuery).bind(
            newGraphId,
          ...
          const insertHistoryQuery = `
            ...
            VALUES (?, ?, ?, ?)
          `;
          await env.vegvisr_org.prepare(insertHistoryQuery).bind(
            crypto.randomUUID(),
            // History entry ID
            newGraphId,
            // Graph ID
      ...
      if (pathname === "/insertWorkNoteIntoGraph" && request.method === "POST") {
        try {
          ...
          const updateQuery = `
            ...
            WHERE id = ?
          `;
          await env.vegvisr_org.prepare(updateQuery).bind(JSON.stringify(graphData), (/* @__PURE__ */ new Date()).toISOString(), graphId).run();
        ...
        try {
          ...
          for (const row of results) {
            ...
            if (graphData.metadata && typeof graphData.metadata === "object" && graphData.metadata.metaArea !== "") {
              ...
              await env.vegvisr_org.prepare(updateQuery).bind(JSON.stringify(graphData), (/* @__PURE__ */ new Date()).toISOString(), row.id).run();
              updated++;
            } else {
              skipped++;
            }
...
Focus on classic Cloudflare Workers patterns and requirements. Do NOT recommend ESM or export default syntax.`;
          const aiResponse = await env.AI.run("@cf/meta/llama-3.1-8b-instruct", {
            messages: [
              {
                role: "system",
                content: "You are an expert Cloudflare Workers developer and code analyst. Provide detailed, actionable feedback on worker code quality, security, and best practices."
              },
              {
                role: "user",
          ...
          const analysis = {
            ...
            deployment_ready: !analysisText.toLowerCase().includes("error") && !analysisText.toLowerCase().includes("critical")
          };
...
DO NOT use ESM or export default syntax. Return ONLY the corrected JavaScript code in classic format.`;
            try {
              const improvementResponse = await env.AI.run("@cf/meta/llama-3.1-8b-instruct", {
                messages: [
                  {
                    role: "system",
                    content: "You are an expert Cloudflare Workers developer. Fix code issues and return clean, working JavaScript code."
                  },
                  {
                    role: "user",
      ...
      if (pathname === "/generate-share-summary" && request.method === "POST") {
        try {
          ...
          const nodeContents = graphData.nodes.filter((node) => node.visible !== false).map((node) => {
            const parts = [];
          ...
          const allContent = [graphTitle, graphDescription, categories, ...nodeContents].join("\n").trim();
...
If content is in Norwegian, use Norwegian phrases like "Se her! Jeg tror dette kan v\xE6re interessant for deg"
...
Return ONLY the social media summary text, no explanations or metadata.`;
            ...
            if (!summary || summary.length < 10) {
              ...
              const isNorwegian = /\b(og|er|på|med|til|av|for|som|ikke|det|en|et|jeg|du|han|hun|vi|de)\b/gi.test(
                allContent
              );
              if (isNorwegian) {
                summary = `Se her! Jeg tror dette kan v\xE6re interessant for deg. ${graphTitle ? `Dette er en kunnskapsgraf om "${graphTitle}"` : "Dette er en interessant kunnskapsgraf"} med ${graphData.nodes.length} noder og ${graphData.edges.length} forbindelser. ${graphDescription || "Den inneholder verdifull informasjon som kan v\xE6re nyttig."}`;
              } else {
            ...
            let fallbackSummary;
            if (isNorwegian) {
              fallbackSummary = `Se her! Jeg tror dette kan v\xE6re interessant for deg. ${graphTitle ? `Dette er en kunnskapsgraf om "${graphTitle}"` : "Dette er en interessant kunnskapsgraf"} med ${graphData.nodes.length} noder og ${graphData.edges.length} forbindelser. ${graphDescription || "Den inneholder verdifull informasjon som kan v\xE6re nyttig."}`;
            } else {
...
Return the complete worker code as plain JavaScript:`;
            ...
            if (!generatedCode.includes("addEventListener") || !generatedCode.includes("handleRequest")) {
...
function extractIssues(analysisText) {
  ...
  lines.forEach((line) => {
    ...
    if (lowerLine.includes("error") || lowerLine.includes("issue") || lowerLine.includes("problem")) {
      issues.push({
        type: lowerLine.includes("syntax") ? "syntax" : lowerLine.includes("security") ? "security" : "general",
        message: line.trim(),
        severity: lowerLine.includes("critical") ? "high" : lowerLine.includes("warning") ? "medium" : "low"
      });
    }
  });
  return issues.slice(0, 10);
}
__name(extractIssues, "extractIssues");
function extractRecommendations(analysisText) {
  const recommendations = [];
  const lines = analysisText.split("\n");
  lines.forEach((line) => {
    const lowerLine = line.toLowerCase();
    if (lowerLine.includes("recommend") || lowerLine.includes("should") || lowerLine.includes("consider")) {
...
__name(extractRecommendations, "extractRecommendations");
function calculateOverallScore(code, analysisText) {
  ...
  if (code.includes("export default")) score -= 40;
  if (code.includes("export {")) score -= 40;
  if (code.includes("import ")) score -= 30;
...
__name(generateSimpleWorkerFromPrompt, "generateSimpleWorkerFromPrompt");
